...
function ver_relacionados(idrep)
{
  ...
  function dame_carrito_transferido(){

    let vendedor_id = $('#usuario_id').val();
    // Petición
    var url="{{url('ventas/damecarritotransferir')}}"; //fragm.ventas_carrito
      $.ajax({
       type:'GET',
       beforeSend: function () {
          $("#carrito").html("Buscando Carrito...");
  ...
  function cargar_carrito_completo_transferir(nombre, usuarios_id)
  {
    let vendedor_id = 
    Vue.swal({
        title: '¿Está seguro?',
        ...
        confirmButtonText: 'ACEPTAR'
            ...
            if (result.value) {
                var url="{{url('ventas/cargarcarritocompleto_transferido')}}"+"/"+nombre+"/"+usuarios_id;
                $.ajax({
                type:'GET',
                url:url,
                success:function(resp){
...
function transferir_carrito(){
  ...
  let hay_carrito = $('#items_carrito').val();
  console.log(hay_carrito);
  if(hay_carrito == 0){
    ...
    $('#transferir-modal').on('shown.bs.modal', function () {
...
function transferir_carrito_confirmar(){
  ...
  let usuarios_id = $('input:radio[name=flexRadioDefault]:checked').val();
  let nombre = $('#nombre_carrito').val();
  let url="{{url('ventas/cargarcarritocompleto_transferido')}}"+"/"+nombre+"/"+usuarios_id;
...
function guardar_venta()
{
  ...
  function habilitar_nombre_carrito(){
    $('#nombre_carrito').removeAttr('disabled');
  }
...
<div class="container-fluid">
    <div class="btn-toolbar" role="toolbar" style="margin-bottom:3px;background-color:peachpuff">
        <div class="btn-group btn-group-sm mr-10" role="group">
          @if (Auth::user()->rol->nombrerol === "Administrador" || Auth::user()->rol->nombrerol === "Cajer@")
            @if(Auth::user()->rol->nombrerol !== "vendedor" ) <button class="btn btn-danger" onclick="nueva_venta()">Nueva Venta</button> @endif
          ...
          @elseif(Auth::user()->rol->nombrerol === "bodega-venta")
          <button type="button" class="btn btn-primary btn-sm" onclick="buscar_cliente()">Buscar Cliente</button>
          @endif
        ...
        <div class="btn-group btn-group-sm ml-auto" role="group">
            <button type="button" class="btn btn-success" id="btn_transferir_carrito" onclick="transferir_carrito()"><small>Transferir Carrito</small></button>
        ...
        @elseif(Auth::user()->rol->nombrerol === "Cajer@")
        <div class="btn-group btn-group-sm ml-auto" role="group">
          <button type="button" class="btn btn-info" onclick="mostrar_nombres_carrito()" >Recuperar Carrito</button>
          <button type="button" class="btn btn-warning" onclick="poner_nombre_carrito()" >Guardar Carrito</button>
          <button type="button" class="btn btn-danger" onclick="borrar_carrito('actual')"><small>Borrar Carrito Actual</small></button>
      </div>
      @elseif(Auth::user()->rol->nombrerol === "vendedor")
      <button type="button" class="btn btn-warning" onclick="poner_nombre_carrito()" >Guardar Carrito</button>
      <button type="button" class="btn btn-info" onclick="mostrar_nombres_carrito()" >Recuperar Carrito</button>
      <button type="button" class="btn btn-danger" onclick="borrar_carrito('actual')"><small>Borrar Carrito Actual</small></button>
...
<div class="row">
    <div class="col-sm-3" style="background-color: #cfcaff;height:60%; padding-bottom: 30px;">
    ...
    <div class="col-sm-9">
        <div  id="carrito" style="background-color: #fffded;min-height: 400px">
        ...
        <fieldset disabled>
            <div class="mt-3" style="display:inline">
                Ingreso por Código:
                <input type="text" id="input_codigo" placeholder="Código" style="width:100px" onkeyup="enter_codigo(event)">
                <input type="text" id="input_cantidad" placeholder="Cant." style="width:50px" onKeyPress="return soloNumeros(event)" onKeyup="enter_cant(event)">
                <button class="btn btn-primary btn-sm" onclick="agregar_carrito(id_repuesto_codigo,3)">AGREGAR</button>
    ...
    <!-- Con este input guardo el id del usuario autenticado para poder transferir el carrito a la cajera con sus propios datos -->
    <input type="hidden" name="" value="{{Auth::user()->id}}" id="usuario_id">
</div>
...
<!-- MODAL TRANSFERIR CARRITO -->
<div class="modal fade" tabindex="-1" id="transferir-modal" data-backdrop="static" data-keyboard="false">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        Transferir carrito
      ...
      <div class="modal-body">
        <ul>
            ...
            <div class="col-md-2">
              <img src="{{url('usuarios/avatar/'.$cajero->image_path)}}" alt="" id="logo">
            </div>
              <div class="col-md-10">
                <div class="form-check">
                  <input class="form-check-input" type="radio" name="flexRadioDefault" onclick="habilitar_nombre_carrito()" value="{{$cajero->id}}" id="flexRadioDefault{{$cajero->id}}">
                  <label class="form-check-label" for="flexRadioDefault{{$cajero->id}}">
                    {{$cajero->name}}
                  </label>
                </div>
              </div>
      ...
      <div class="modal-footer">
        <input type="text" class="form-control" id="nombre_carrito" placeholder="nombre del carrito" disabled>
        <button type="button" class="btn btn-danger" data-dismiss="modal" onclick="">Cancelar</button>
        <button type="button" class="btn btn-primary" onclick="transferir_carrito_confirmar()">Aceptar</button>
      </div>
    </div>
  </div>
</div>
<!--  FIN MODAL TRANSFERIR CARRITO-->
