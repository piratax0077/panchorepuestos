        ...
        @foreach ($detalle as $item)
            @if($mot==1) <!-- Anular documento completo -->
                <tr>
                    ...
                    <td align="center" id="descuento-{{$item->id}}">{{number_format($item->descuento*(1+Session::get('PARAM_IVA')),0,',','.')}}</td>
                    <td align="center" id="total-{{$item->id}}">{{number_format(($item->precio_venta*$item->cantidad)-($item->descuento*(1+Session::get('PARAM_IVA'))),0,',','.')}}</td>
                </tr>
                @php
                    $subtota+=$item->precio_venta*$item->cantidad;
                    $dcto+=$item->descuento*(1+Session::get('PARAM_IVA'));
                @endphp
            @endif
